...
// 获取并显示免责声明(需传入$实例)
async function disclaimer($){const u=['https://fastly.jsdelivr.net/gh/czy13724/Quantumult-X@main/NAvailable/Declaration.json','https://fastly.jsdelivr.net/gh/czy13724/Quantumult-X@main/NAvailable/Description.json'],t=2e3,f=x=>new Promise(r=>{let d=0,e=v=>{d||((d=1),r(v))},m=setTimeout(()=>e(null),t);try{typeof $task!='undefined'?$task.fetch({url:x}).then(o=>{clearTimeout(m);try{e(JSON.parse(o.body))}catch(_){e(null)}},_=>{clearTimeout(m);e(null)}):typeof $httpClient!='undefined'?$httpClient.get({url:x,timeout:Math.ceil(t/1e3)},(o,_,b)=>{clearTimeout(m);if(o||!b)return e(null);try{e(JSON.parse(b))}catch(_){e(null)}}):(require('https').get(x,o=>{let b='';o.on('data',c=>b+=c);o.on('end',()=>{clearTimeout(m);try{e(JSON.parse(b))}catch(_){e(null)}})}).on('error',()=>{clearTimeout(m);e(null)}))}catch(_){clearTimeout(m);e(null)}});for(const x of u){const r=await f(x);if(r&&r.notice){console.log(r.notice);break}}}
